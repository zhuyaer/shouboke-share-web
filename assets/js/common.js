/**
 * Created by laikanz on 16/11/29.
 */
...
function getCard(){//获取邀请卡信息
    ...
    $.ajax({
        ...
        success:function(data){
            $('#coupon')[0].innerHTML=data.discountPercent+'折优惠券';
            $("#original").html('原价：'+data.stream.charge+"元");
            if(data.discountPercent==0){
                $('#coupon')[0].innerHTML='免费优惠券';
            }
            cardMoney=data.money;
            cardData=data;
            sessionStorage.setItem('cardInfo',JSON.stringify(data));
            status=data.status;
            $('#present span')[1].innerHTML=data.price;
            console.log('---',!data.has&&data.status!='expired')
            if(!data.has&&data.status!='expired'&&data.discount){
                sessionStorage.setItem('hasCard',true);
                $('.charge')[0].innerHTML='¥'+cardData.price;
                showCard();
            }else{
...
function createCard(){
    ...
    $.ajax(_url+'/v4/card/customer/create',{//获取token
        data:{cardId:cardId},
        ...
        success:function(data){
            ...
            if(data.error){
                ...
                time=data.seconds;
                cardData=data;
                $('#oldPrice')[0].value='¥'+data.stream.charge;
                $('#coupon')[0].value='- ¥'+(data.stream.charge-data.price).toFixed(2);
                if(data.price){
                    $('#realPay')[0].innerHTML='¥'+data.price;
                }else{
                    $('#realPay')[0].innerHTML='¥'+data.stream.charge;
                }
...
function goMsgBottom($parent,$child){
    //var messageBox = $('#messageBox');
    //var msg = $('ul#msg li:last-child');
    $parent.scrollTop(
        $child.offset().top-$parent.offset().top+$parent.scrollTop()
    );
}
String.prototype.httpHtml = function(){
        var reg = /(http:\/\/|https:\/\/)?(\w+\.(\w|=|\?|\.|\/|&|-)+)/g;
        return this.replace(reg,function(regv){
            if(regv.indexOf('http')==-1){
                return '<a href="http://'+regv+'" target="_blank">'+regv+'</a>'
            }else{
                return '<a href="'+regv+'" target="_blank">'+regv+'</a>'
            }
        })
};
