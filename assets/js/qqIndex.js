...
(function($){
    var Page={
        ...
        // wxPayUrl:"http://shouboke.tv/weChat_web_develop/class/html/h5wxPay.html",
        wxPayUrl:"http://shouboke.tv/class/html/h5wxPay.html",
        // bindUrl:"http://shouboke.tv/weChat_web_develop/class/html/bindPhone.html",
        bindUrl:"http://shouboke.tv/class/html/bindPhone.html",
        // qqUrl:'https://graph.qq.com/oauth2.0/authorize?response_type=token&client_id=1105569906&redirect_uri=http://shouboke.tv/weChat_web_develop/qqIndex.html?state=',
        qqUrl:'https://graph.qq.com/oauth2.0/authorize?response_type=token&client_id=1105569906&redirect_uri=http://shouboke.tv/shareModel/qqIndex.html?state=',
        // recommendUrl:'http://shouboke.tv/webChatPublic/developQqRecommend.html',
        recommendUrl:'http://shouboke.tv/webChatPublic/qqRecommend.html',
        /******************生产环境配置END***************/
        ...
        getVideoInfo:function(){
            ...
            if(!_this.dataObj.streamId){
                _this.dataObj.streamId=getQueryString('state')||sessionStorage.getItem('streamId');
            }
        ...
        shoubokeCharge:function(){
            ...
            if(_this.ifCharge){//收费课
                if(cardData.discount!=100){ //优惠券折扣不为100%
        ...
        getHotDta:function(){
            ...
            var liveurl=_this.commonUrl+'/v4/stream/relatedStreams?page='+_this.hotVideoList.livePage+'&resultsPerPage='+_this.hotVideoList.liveperPageCount+'&userid='+_this.dataObj.userid+'&category='+_this.category;
            //－－－－－－－－－－－－－－－－－－－－－－精彩预告
            // $.ajax({
            //     type:'GET',
            //     url:readyurl,
                ...
                success:function(data){
                    ...
                    if(data.items.length==0){
                        ...
                        for(var i=0;i<data.items.length;i++){
                            opt+='<li>'+
                            '<div class="test-img"><a href="'+_this.qqUrl+data.items[i].streamId+'"><img src="'+data.items[i].thumbnailUrl+'"/></a></div>'+
    ...
    $(function(){
        var url = location.href;
            ...
            function init_to(){
                ...
                var refresh=sessionStorage.getItem('refresh');//判断是否进入付费页面，进入则刷新原页面
                if(refresh){
                    sessionStorage.setItem('refresh','');
                    setTimeout(function(){
                        location.reload();
            ...
            if(QC.Login.check()){//判断是否已经获得授权
                        ...
                        success:function(data){
                            ...
                            $.ajax({
                                ...
                                success: function (data){
                                    ...
                                    uPhone=data.phone;
                                    sessionStorage.setItem('phoneFlag',uPhone);//记录电话号码到本地，避免从绑定页面回退时报名操作报错
                                    if(cardId){
                ...
                window.location.href=Page.qqUrl+streamId+'&scope=get_user_info';
            }
            //针对未授权登录客户新增授权链接
