...
(function($){
    var Page={
        ...
        //默认链接
        // linkUrl:"https://api.weibo.com/oauth2/authorize?client_id=367727881&response_type=code&redirect_uri=http://shouboke.tv/weChat_web_develop/weiboIndex.html?streamId=",
        linkUrl:"https://api.weibo.com/oauth2/authorize?client_id=367727881&response_type=code&redirect_uri=http://shouboke.tv/shareModel/weiboIndex.html?streamId=",
        // bindUrl:"http://shouboke.tv/weChat_web_develop/class/html/bindPhone.html",
        bindUrl:"http://shouboke.tv/class/html/bindPhone.html",
        // recommendUrl:'http://shouboke.tv/webChatPublic/developWbRecommend.html',
        recommendUrl:'http://shouboke.tv/webChatPublic/weiboRecommend.html',
        // wbUrl:'http://shouboke.tv/weChat_web_develop/weiboIndex.html?streamId=',
        wbUrl:'http://shouboke.tv/shareModel/weiboIndex.html?streamId=',
        /******************生产环境配置END***************/
        ...
        getTicket:function(){
            var _this=this;
            $.ajax({
                url: _this.commonUrl+"wechat/getWeChatTicket",  //获取视频信息以及权限
                type:"post",
        ...
        wxShare:function(data){
            ...
            if(data){
                ...
                var shareData={//分享到朋友
                        ...
                        // link: _this.linkUrl+_this.dataObj.streamId+'#wechat_redirect', // 分享链接，默认分享当前页，目前微信分享不支持https格式的链接
                ...
                wx.ready(function(){
                    wx.onMenuShareTimeline({//朋友圈
                        title:"手播课", // 分享标题
                        //link: _this.linkUrl+_this.dataObj.streamId+"#wechat_redirect", // 分享链接
        ...
        loadVideoInfo:function(data){
            ...
            if(freeNewBind&&!data.isSubscribe){
                _this.siGnSuccess();
            }
        ...
        shoubokeCharge:function(){
            ...
            if(_this.ifCharge){//需要收费
                $("#payMoney.ui-dialog").dialog("show");
                 // location.href='http://shouboke.tv/weChat_web_develop/class/html/h5wxPay.html';
                // var dia=$.dialog({
                //     title:'温馨提示',
                //     content:'请关注手播课微信公众号报名，或点击左上角下载APP'
                // });
            }else{
                _this.siGnSuccess();
                return;
        ...
        siGnSuccess:function(){
            ...
            var token=sessionStorage.getItem('token');
            if(_this.psdCourse){
                postData={streamId:dataObj.streamId,password:_this.psdCourse}
            }else{
        ...
        getHotDta:function(){
            ...
            var liveurl=_this.commonUrl+'/v4/stream/relatedStreams?page='+_this.hotVideoList.livePage+'&resultsPerPage='+_this.hotVideoList.liveperPageCount+'&userid='+dataObj.userid+'&category='+_this.category;
            //－－－－－－－－－－－－－－－－－－－－－－精彩预告
            // $.ajax({
            //     type:'GET',
            //     url:readyurl,
                ...
                success:function(data){
                    ...
                    if(data.items.length==0){
                        ...
                        for(var i=0;i<data.items.length;i++){
                            opt+='<li>'+
                            '<div class="test-img"><a href="'+_this.linkUrl+data.items[i].streamId+'"><img src="'+data.items[i].thumbnailUrl+'"/></a></div>'+
        ...
        bindEvent:function(){
            ...
            $("#msgInfo").on('focus',function(){//点击消息框判断用户是否绑定手机号
                var phoneFlag=sessionStorage.getItem('uPhone');
                if(!phoneFlag) {
            ...
            $(".liveListTitle .findMore").on('click',function(){
                window.location.href=_this.recommendUrl;
            })
            //切换到举报
    ...
    $(function(){
        ...
        var url = location.href;
                    ...
                    success: function (data){
                        ...
                        for(var i=0;i<data.thirdpart.length;i++){
                            if(data.thirdpart[i].type=="wb"){
                                ...
                                dataJson=JSON.stringify(dataJson)
                                sessionStorage.setItem('dataJsonSession',dataJson);
                                sessionStorage.setItem('pageType','weibo');
            ...
            function postCode(){//使用code换取access_token
                var _this=this;
                var code=getQueryString('code');
                var streamId=getQueryString('streamId');
                $.ajax({
                    url: Page.commonUrl+"/weibo/token",
                    type: "get",
                    data: {
                        code: code,
                        url: Page.wbUrl+streamId
                    },
                    success: function (data){
                            getUserInfo(data);
                        },
