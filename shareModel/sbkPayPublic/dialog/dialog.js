(function(){
    ...
    var util = {
        ...
        genIcon: function (icon) {
            //默认无icon,true为默认icon
            if (!icon) return "";
            return '<p class="inke-icon ' + icon + '"></p>';
        },
        ...
        genTip: function (opts) {
            ...
            if(tip.txt){
                if (title.txt) {
                    ...
                    tip.size = tip.size || '14px';
                } else {
                    tip.color = tip.color || "#333";
                    tip.size = tip.size || '16px';
                }
    ...
    d.alert = function (cfg) {
        ...
        dialog = Dialog({
            type: "alert " +( opts.type ? opts.type : "" ),
            icon: "inke-icon-alert",
            wallCss: "",
            wrapCss: "", //background: #fff;width: 267px;text-align: center;
            title: {
                txt: opts.title
            },
            tip: {
    ...
    d.confirm = function (cfg) {
        ...
        dialog = Dialog({
            type: "confirm " + (opts.type ? opts.type : ""),
            title: {
    ...
    d.loading = function (cfg) {
        ...
        dialog = Dialog({
            type: "loading " + (opts.type ? opts.type : ""),
            wallCss: "",
    ...
    d.flatLoading = function (cfg) {
        ...
        dialog = Dialog({
            type: "floading " + (opts.type ? opts.type : ""),
    ...
    d.tip = function (cfg) {
        ...
        dialog = Dialog({
            type: "tip ",
            icon: cfg.icon || "inke-icon-alert",
            wallCss: "background:#fff;",
            wrapCss: cfg.wrapCss || "width: 140px; height:140px; background-color: rgba(0,0,0,0.8); color: #fff;",
            tip: cfg.tip || {
                txt: _cfg.text || "温馨提醒",
                color: "#adadad",
                size: "14px"
    ...
    var lycam = window.Lycam||{};
    ["alert","confirm","loading","tip"].forEach(function(key,i){
        lycam[key] || (lycam[key] = d[key]);
    });
    window.Lycam = lycam;
})();
